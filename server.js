...
app.get('/api/price', async (req, res) => {
  ...
  // Progressive backoff: 5min → 15min → 30min → 1hr based on failures
  const baseCache = 5 * 60 * 1000; // 5 minutes
  const backoffMultiplier = Math.min(priceCache.failedAttempts, 12); // cap at 12
  const CACHE_MS = baseCache + (backoffMultiplier * 5 * 60 * 1000);

  // Return cached if fresh (survives server restarts by checking timestamp)
  ...
  try {
    ...
    // Return last known price, update lastFetch to prevent immediate retry
    ...
    res.json({
      ...
      cached: true,
      stale: true,
      backoffMinutes: Math.round(CACHE_MS / 60000),
      error: err.message.includes('429') ? 'Rate limited by CoinGecko' : err.message
    });
  }
});

// ============================================
// CONFIG.JSON ROUTE (Lucifer requires this at root)
// ============================================
