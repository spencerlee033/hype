...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https://cdnjs.cloudflare.com", "https://cdn.tailwindcss.com", "https://s3.tradingview.com"],
      imgSrc: ["'self'", "data:", "https:"],
      connectSrc: ["'self'", "https://api.binance.com", "https://fapi.binance.com", "https://api.coingecko.com"],
      frameSrc: ["'self'"],
...
// Rate limiting
// Trust proxy (Render sits behind proxies)
...
// Explicit /files route with CORS headers for cross-origin access
// Handles Lucifer scripts and other static assets from files/ folder
app.use('/files', (req, res, next) => {
  ...
  setHeaders: (res, filePath) => {
    // Ensure JS module files are served with correct MIME type
...
// ============================================
// LIVE PRICE API (CoinGecko proxy with cache)
// ============================================
...
app.get('/api/price', async (req, res) => {
  const now = Date.now();
  const CACHE_MS = 5 * 60 * 1000; // 5 minutes to avoid CoinGecko 429

  // Return cached if fresh
  if (now - priceCache.lastFetch < CACHE_MS) {
    return res.json({
  ...
  try {
    const response = await axios.get(
      ...
      { 
        ...
        headers: {
          ...
          'User-Agent': 'Mozilla/5.0 (compatible; HYPE-Tracker/1.0)'
        }
      }
    );

    const data = response.data.hyperliquid;
    ...
    // Return last known price (never reset to 41.65 if we have a cached price)
    res.json({
      price: priceCache.price,
      change: priceCache.change24h,
      cached: true,
...
// ============================================
// STATIC FILE FALLBACKS (for Lucifer cross-origin loading)
// ============================================

// Serve config.json from ROOT (where it actually lives) with CORS
...
// Also serve at /files/config.json in case Lucifer looks there
app.get('/files/config.json', (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS');
  res.sendFile(path.join(__dirname, 'config.json'));
});
